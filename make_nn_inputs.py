...
- Optional "horizon" feature if trainer used INCLUDE_H_AS_FEATURE=true
...
def load_static_encoder(models_dir, product_df):
    ...
    If the file is missing or unreadable/corrupted, fall back to fitting a new encoder.
    """
    ...
    try:
        if enc_path.exists():
            with open(enc_path, "r") as f:
                blob = json.load(f)  # may raise JSONDecodeError
            cols = blob["columns"]
...
def build_base_features_for_t(i, t, LB, sales_np, stock_np,
    ...
    """
    Rebuild the exact per-(i,t) feature vector used in training,
    EXCLUDING the optional horizon feature (added later per-h).
    """
    eps = 1e-6
    # Inclusive lag window [t-LB+1 : t]
    ps = sales_np[i, t - LB + 1 : t + 1].astype(np.float32)  # [LB]
    pa = stock_np[i, t - LB + 1 : t + 1].astype(np.float32)  # [LB]
    if ps.shape[0] != LB or pa.shape[0] != LB:
        raise RuntimeError(f"Lag window mismatch at t={t}")
    ...
    # Concatenate in the SAME ORDER as training feature_names
    base = np.concatenate([
        ps,                  # sales_lag_*
        pa,                  # avail_lag_*
...
def main():
    ...
    # Align timeline consistent with training
    ...
    date_df_eff = date_df.iloc[:T_eff].reset_index(drop=True)

    # Time-product features
    tp = time_product_features.numpy()[:, :, 0, :T_eff + maxH] if (has_tp and use_tp) else None
